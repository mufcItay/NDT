...
% load the data
%itay data = str2fullfile(fullfile(dataPath,subjectID),['2018*' subjectID '*' '.mat']);
data = dir(strcat(fullfile(dataPath,subjectID),'\2018*.mat'));
fns = {data.name};
dirs = {data.folder};
for d = 1:length(data)
    %itay load(data{d},'B','P')
    load(strcat(dirs{d},'\',fns{d}),'B','P');
    if d == 1
        V        = B(:,1);
        response = B(:,5);
...
%itay
subjData = table();
% remove mixed trials from anaylsis
subjData.iv = P(D~=.5);
subjData.dv = D(D~=.5);
subjData.subNum(:) = str2num(subjectID(2:end));
writetable(subjData, strcat('..\..\Converted\',subjectID,'.csv'))
%itay-end
