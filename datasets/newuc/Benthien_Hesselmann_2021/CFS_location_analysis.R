...
#mean RT visibility
means_vis <- round(data_summary(data_main, varname="RT",
                  groupnames=c("visibility")),3)
 
#mean RT separately for all conditions
means <- round(data_summary(data_main, varname="RT", 
                  groupnames=c("visibility", "certainty", "congruency")),3)
...
p <- ggplot(means_cfs, aes(x=certainty, y=RT, fill=congruency)) +
  ...
  geom_errorbar(aes(x=certainty, ymin=RT-sd, ymax=RT+sd), width=.1,
