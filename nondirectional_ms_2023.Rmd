...
authornote: |
  All datasets and analysis scripts used in this manuscript are publicly available online: https://github.com/mufcItay/NDT. 
...
# groundhog.library(c(
#   'github::mufcItay/signcon'), '2023-09-09')
library(signcon)
...
# appendixA_res <- run_appendixB(apndxB_conf_FAs, apndxB_conf_sensitivity)
appendixA_res <- read.csv('appendix\\results\\2023-12-13_21_01_53Appendix_A_QUID_OANOVA.csv')
...
appendixC_res <- read.csv('appendix\\results\\2023-12-06_22_35_58Appendix_B.csv')
save_plot_appendixB(appendixC_res)
...
appendixF_res <- read.csv('appendix\\results\\2023-12-09_11_21_53Appendix_E.csv')
...
# Appendix A. # Simulating non-directional unconscious effects with Wald distributions  {#Apndx-A}

To examine the robustness of our approach to  
...
appendix_B_analysis <- analyze_appendix_A(appendixA_res)
...
apndxC_sig_b <- dplyr::first(apndx_b_conf$params$sigma_b)
apndxC_sig_w <- dplyr::first(apndx_b_conf$params$sigma_w)
# we multiply by 2 because the simulation expects number of trials per condition and we report the total amount of trials
apndxC_Nt <- 2 * unique(apndx_b_conf$params$N_t)
apndxC_Np <- unique(apndx_b_conf$params$N_p)
apndxC_de_mu <- max(apndx_b_conf$params$mu)
apndxC_nd_mu <- min(apndx_b_conf$params$mu)
apndxC_iterations <- max(apndx_b_conf$results$seeds)
```


# Appendix B. Comparing the power of the GNT and sign consistency test {#Apndx-B}
...
knitr::include_graphics(paste("appendix","Appendix_B_Differences.png", sep = .Platform$file.sep))
```

\newpage

# Appendix D. Extending the sign consistency test to additional use cases
...
knitr::include_graphics(paste("appendix","Appendix_D.png", sep =
                                .Platform$file.sep))
```
...
# Appendix F. Empirically informed power sign-consistency test power estimation
...
To that end, we simulated *non-directional differences* scenarios with different degrees of true between participants variance ($\sigma_b \in \{1, 1.5, 2\}$) and fixed amount of within participant variability ($\sigma_w = 10$). These parameters were chosen based on previous works where the ratio $\frac{\sigma_b}{\sigma_w}$ was estimated to values ranging between .04 and .15 (M = .1, SD = .04) in six unconscious processing datasets [@meyen2022a], while in another work estimating the same parameter in 24 cognitive control studies [@rouder2023a] (where no unconscious manipulation was used) values ranges between .05 and .36 (M = .14, SD = .08). Then, we determined the number of participants and subjects in the simulated datasets according to the sample sizes in the unconscious processing datasets we obtained by calculating the 25%, 50%, and 75% percentile of both parameters. This resulted in additional simulation conditions where the number of trials was set to $N_p \in \{`r apndxE_N_p[1]`, `r apndxE_N_p[2]`, `r apndxE_N_p[3]`\}$ and the total number of trials was set to $N_t \in \{`r apndxE_N_t[1]`, `r apndxE_N_t[2]`, `r apndxE_N_t[3]`\}$. All other simulation parameters were the same as detailed in Appendix B.

From the results of this power simulation we obtained an estimate for the power of the sign consistency test, in common sample size settings, for an effect of interest of $\frac{\sigma_b}{\sigma_w}=.15$ (the same ratio was used in @meyen2022a as part of a "benefit of a doubt" approach for a different set of analyses on unconscious processing effects). We then used a prevalence test to test the obtained power estimate of `r round(100 * apndx_E_median_cnd_power,0)`% is compatible with the low observed prevalence of unconscious processing effects we found (`r round(sc_sig_n/n_empiric_ns,2)`%). Specifically, we tested if the observed proportion of significant sign consistency effects is lower than the expected prevalence according to our power estimate (`r round(100 * apndx_E_median_cnd_power,0)`%, as stated above). Indeed, this was the case when comparing the observed rate of sifnificant effects with the expected rate if all, or even half of the effects exist, yet were not detected due to a lack of power ($CI_{95}$ = [`r round(100 * test_full_prevalence$conf.int[1],2)` `r round(100 * test_full_prevalence$conf.int[2],2)`], and p = `r apa_p(test_full_prevalence$p.value)`, p = `r apa_p(test_half_prevalence$p.value)`, respectively). Hence, we interpret these results as suggesting that for an effect size of $\frac{\sigma_b}{\sigma_w}=.15$ the results are unlikely to be explained simply by lack of power.

(ref:appendixecaption) Appendix F-figure 1. Power analysis for the sign consistency (SC) test in various *non-directional differences* scenarios. Each panel depicts the results for different effect size ($\frac{\sigma_b}{\sigma_w} \in \{.1, .15, .2\}$). Within each panel, the x and y axes depict different settings for the number of participants ($N_p$) and the total number of trials across two conditions ($N_t$), both determined according to the 25, 50 and 75 percentiles of these parameters in the unconscious processing datasets we collected. The number in each cell denotes the % of significant effects (power) in each simulated condition (across 1000 iterations), and darker blue colors indicate higher power.
...
* **Which test should be used?**
    ...
    + To test for the prevalence of individual-level effects, rather than the mere existence of an effect at the group level, we recommend using the prevalence approach [@ince2021a; @ince2022a]. More specifically, we recommend using the GNT [@donhauser2018a] to test whether the data provide evidence for the presence of an effect for at least a single individual.
