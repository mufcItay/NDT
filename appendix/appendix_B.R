...
appendix_b_conf <- initialize_simulation(N_p, N_t, sigma_b, sigma_w, mu, max_seed, 
...
run_appendixB <- function(conf, alpha = .05) {
  ...
  return(list(summary = results_summary, full_res = results_df))
}

save_results_plot <- function(results_summary) {
  # plot the results
